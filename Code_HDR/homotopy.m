...
% and eigenvector for this value should be given in eigenval_0 and 
...
while s<1 && ind_eig>1 %%% HOMOTOPY
    ...
    % Now most things have to be rigorous
    is = intval(s);
    iLs = is*op.iL + (1-is)*op.iL0;
    iv = [intval(eigenvect_s); zeros(2*N,ind_eig-1)];
    iLv = iLs*iv;
        
    iA0 = iv'*op.iMat_scal*iv;
    iA1 = iLv'*op.iMat_scal*iv;
    
    [upper_vect_Ls, upper_eig_Ls] = eig(iA1.mid,iA0.mid); %Numerical upper bounds
    [upper_eig_Ls, ind_upper_eig_Ls] = sort(real(diag(upper_eig_Ls)));
    upper_vect_Ls = upper_vect_Ls(:,ind_upper_eig_Ls);
    all_eigs = true;
    iupper_eig_s = validate_eigenvalues(upper_eig_Ls, upper_vect_Ls, iA1, iA0, para.tol_simple_eig, all_eigs);
    ...
    if iupper_eig_s(ind_eig-1) < ilower_eig_s(ind_eig)
        ...
        iB1 = iA1-inu*iA0;
        iB2 = iLnuv'*op.iMat_scal*iLnuv; 
        [lower_vect_Ls, lower_eig_Ls] = eig(iB2.mid, iB1.mid); %Numerical lower bounds
        [lower_eig_Ls, ind_lower_eig_Ls] = sort(real(diag(lower_eig_Ls)),'descend');
        lower_vect_Ls = lower_vect_Ls(:, ind_lower_eig_Ls);
        
        all_eigs = false;
        if s==1
            all_eigs = true;
        end
        ilower_eig_s = validate_eigenvalues(lower_eig_Ls, lower_vect_Ls, iB2, iB1, para.tol_simple_eig, all_eigs);        
        ...
        if para.show_enclosure %|| s==1
            fprintf('\nEnclosure for the eigenvalues at s = %f\n',s)
            infsup(infsup(ilower_eig_s, iupper_eig_s))
        end
    else
    ...
    if s<1
        ...
        if ind_eig > 2 && abs((iupper_eig_s(ind_eig-1)-ilower_eig_s(ind_eig))/ilower_eig_s(ind_eig)) < 1.5*margin
            ...
            if para.show_enclosure
                fprintf('\nEnclosure for the eigenvalues (now with all the lower bounds) at s = %f\n',s)
                infsup(infsup(ilower_eig_s, iupper_eig_s))
            end
        end
